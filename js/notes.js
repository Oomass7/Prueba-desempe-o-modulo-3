...
<div class="control">
<input class="input" type="number" id="event-amount" placeholder="e.g., 100">
</div>
</div>
<div class="field">
...
<label class="label">Amount</label>
<div class="control">
<input class="input" type="text" id="edit-event-amount"
placeholder="e.g., Virtual, Convention Center">
</div>
</div>
<div class="field">
...
const editEventAmountInput = document.getElementById('edit-event-amount')
...
const amount = parseInt(eventAmountInput.value)
...
if (!name || !date || !location || isNaN(price) || isNaN(amount)) {
alert("Please fill in all event fields correctly.");
return;
}

await storeEvent(name, date, location, price, status, amount, image, description); // NEW: Pass image and description
if (newEventModal) {
newEventModal.classList.remove('is-active');
}
clearEventForm();
...
const amount = parseInt(editEventAmountInput.value)
...
await updateEvent(eventId, name, date, location, price, status, amount, image, description); // NEW: Pass image and description
if (editEventModal) {
editEventModal.classList.remove('is-active');
}
if (eventsTableBody) {
